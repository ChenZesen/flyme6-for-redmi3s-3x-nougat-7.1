...
#-----------------------------------------------------------------------------
vendor_saved_apps := Bluetooth BluetoothExt BluetoothMidiService  EasterEgg ExtShared \
                     HTMLViewer KeyChain PacProcessor PrintRecommendationService PrintSpooler Stk UserDictionaryProvider \
		      WAPPushManager BackupRestoreConfirmation  CarrierConfig \
		      CellBroadcastReceiver DefaultContainerService EmergencyInfo ExternalStorageProvider FMRadio \
		      FusedLocation InputDevices ManagedProvisioning MtpDocumentsProvider ProxyHandler SharedStorageBackup \
		      Shell StatementService StorageManager WallpaperCropper MKSettingsProvider QtiTetherService \
		      qcrilmsgtunnel CMDoze ims QtiTelephonyService datastatusnotification
...
#-----------------------------------------------------------------------------
#board_remove_apps := LogReport MzUpdate MzInput  OperaMax MzCountService MzPhoneLocationService MzSyncService   MzBackup AppCenterIntl WeatherIntl flymebbs MzCloudService NfcNci GameCenter Life IflytekSpeechService Mall MzCompaign Pedometer RoamingPay Reader GameSDKService Search vlife-sdk-mx EBook VoiceAssistant Wallet  ExperienceDataSync MzPay
...
override_property += \
    ro.flyme.romer=lanban \
    ro.product.model_romer=hm3s_lanban
    ro.product.device=land
##############################################################################
